...
int join_table(int table_id_1, int table_id_2, char* pathname) {
    ...
    for (i = 0; i < MAX_FILE_NUM; i++) {
        if (files[i].id == table_id_1) { break; }
    }
    ...
    for (i = 0; i < MAX_FILE_NUM; i++) {
        if (files[i].id == table_id_2) break;
    }
    ...
    // TODO: is layered?
    int jid = open(pathname, O_RDWR | O_CREAT, 0666);

    buffer_frame_t* l1 = get_buf_next_leaf(table_id_1, 0);
    buffer_frame_t* l2 = get_buf_next_leaf(table_id_2, 0);

    int r = 0, s = 0;
    tuplenum_t tuplenum = 0;
    buffer_frame_t* mark;
    int mark_s;

    // TODO: while condition.
    while (l1->page_num != 0 && l2->page_num != 0) {

        while (l1->frame.node.records[r].key < l2->frame.node.records[s].key) {
            r++;
            if (l1->frame.node.num_keys == r) {
                r = 0;
                l1 = get_buf_next_leaf(table_id_1, l1->page_num);
            }
        }

        while (l1->frame.node.records[r].key > l2->frame.node.records[s].key) { 
            s++;
            if (l2->frame.node.num_keys == s) {
                s = 0;
                l2 = get_buf_next_leaf(table_id_2, l2->page_num);
            }
        }
        ...
        while (l1->frame.node.records[r].key == l2->frame.node.records[s].key) {
            ...
            while (l1->frame.node.records[r].key == l2->frame.node.records[s].key) {
                ...
                // yield
                //buffer_write_tuple(jid, l1, r, l2, s, pathname, tuplenum);
                dprintf(jid, "%d.%ld,%d.%s,%d.%ld,%d.%s\n", table_id_1, l1->frame.node.records[r].key,
                                                        table_id_1, l1->frame.node.records[r].value,
                                                        table_id_2, l2->frame.node.records[s].key,
                                                        table_id_2, l2->frame.node.records[s].value);
                tuplenum += 2;
    ...
    /* if result of join is empty,
     * rename the file name
     */
    if (tuplenum == 0) {
        rename(pathname, "result_empty_join");
    }
...
buffer_frame_t* get_buf_next_leaf(int table_id, pagenum_t n) {
    ...
    // n is header, therefore n == 0
    if (n == 0) {

        buffer_frame_t* header = get_buf_frame(table_id, n);
        ...
        // if root exists.
            ...
            buffer_frame_t* root = get_buf_frame(table_id, header->frame.header_page.root);
            unpin(header);
            // if root is leaf.
            if (root->frame.node.is_leaf == 1) {
                return root;
            }
            // if root is internal.
            else {
                buffer_frame_t* node = get_buf_frame(table_id, root->frame.node.right_or_indexing);
                unpin(root);
                while (!node->frame.node.is_leaf) {
                    node = get_buf_frame(table_id, node->frame.node.right_or_indexing);
                }
                return node;
            }
        }
    ...
    // n is leaf, therefore n != 0
    else {
        buffer_frame_t* leaf = get_buf_frame(table_id, n);
        if (leaf->frame.node.right_or_indexing == 0) { 
            unpin(leaf);
            return get_buf_frame(table_id, 0);
        }
        buffer_frame_t* next = get_buf_frame(table_id, leaf->frame.node.right_or_indexing);
        // TODO: unpin adjsent leaves issue...
        unpin(leaf);
        return next;
    }
}
